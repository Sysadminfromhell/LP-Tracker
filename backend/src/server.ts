...
async function loadLeaderboardFromDatabase(): Promise<void> {
...
function getLeaderboard(): LeaderboardPlayer[] {
...
async function refreshPlayer(player: Player): Promise<boolean> {
  console.log(`[REFRESH] ${player.gameName}#${player.tagLine}`);
  try {
    ...
    if (cached) {
      console.log(
        `[REFRESH] ${player.gameName}#${player.tagLine}: ` +
          `${cached.current.tier} ` +
      ...
      console.log(`[REFRESH] ${player.gameName}#${player.tagLine} updated ✓`);
    }
    return true;
  } catch (error) {
...
async function refreshPlayersForSnapshot(players: Player[]): Promise<Player[]> {
